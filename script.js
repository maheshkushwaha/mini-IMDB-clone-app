// Key - 179e7163
//URL - http://www.omdbapi.com/?i=tt3896198&apikey=179e7163
...
fetch('http://www.omdbapi.com/?i=tt3896198&apikey=179e7163')
    .then(res => res.json())
    .then(data => console.log(data));

// Upon keypress - function searchMovies is initiated
searchBox.addEventListener('input', searchMovies);


async function movieDetail() {
    // Finding ID of the movie from the URL
    var urlMovieParams = new URLSearchParams(window.location.search);
    var id = urlMovieParams.get('id')
    console.log(id);
    ...
    document.querySelector('.movie-card-container').innerHTML = output
...
async function searchMovies() {
    const url = `https://www.omdbapi.com/?s=${(searchBox.value).trim()}&page=1&apikey=${key}`
    const res = await fetch(`${url}`);
    const data = await res.json();
